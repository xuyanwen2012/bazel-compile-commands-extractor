...
import enum
# MIN_PY=3.9: Replace `functools.lru_cache(maxsize=None)` with `functools.cache`.
import functools
import itertools
import json
import locale
...
def _log_with_sgr(sgr, colored_message, uncolored_message=''):
    ...
    print(sgr.value, colored_message, SGR.RESET.value,
          uncolored_message, sep='', file=sys.stderr, flush=True)
...
def _print_header_finding_warning_once():
    ...
    if _print_header_finding_warning_once.has_logged:
        return
...
def _parse_headers_from_makefile_deps(d_file_content: str, source_path_for_sanity_check: typing.Optional[str] = None):
    ...
    #dependency-file-generation for more details.
    ...
    assert target.endswith(
        ('.o', '.obj')), "Something went wrong in makefile parsing to get headers. The target should be an object file. Output:\n" + d_file_content
    ...
    # The first dependency is a source entry, only used to (optionally) sanity-check the dependencies if a source path is provided.
    source, *headers = dependencies
    assert source_path_for_sanity_check is None or source.endswith(
        source_path_for_sanity_check), "Something went wrong in makefile parsing to get headers. The first dependency should be the source file. Output:\n" + d_file_content
...
def _get_headers_gcc(compile_action, source_path: str, action_key: str):
            ...
            if arg.startswith('-MF'):
                ...
                if os.path.isfile(dep_file_path):
                    ...
                    if (_get_cached_adjusted_modified_time(source_path) <= dep_file_last_modified
                        ...
                        # Fresh cache! exit early. Still put in the Hedron outer cache bc we're willing to hit stale if we're unable to get new headers.
                        return headers, True
                break
    ...
    if is_nvcc:
        ...
        header_cmd += ['-M', '--print-missing-file-dependencies']
    ...
    if is_nvcc:
        ...
        headers = {
            header for header in headers if _get_cached_file_exists(header)}
...
def _get_headers_msvc(compile_action, source_path: str):
    ...
    for line in header_search_process.stderr.splitlines():
        ...
        # Munching the source filename echoed the first part of the include output
...
def _get_headers(compile_action, source_path: str):
    ...
    cached_headers = None
    if output_file:
        cache_file_path = output_file + \
            ".hedron.compile-commands.headers"  # Embed our cache in bazel's
    ...
    if compile_action.arguments[0].endswith('cl.exe'):
        ...
        headers, should_cache = _get_headers_gcc(
            compile_action, source_path, compile_action.actionKey)
    ...
    if {exclude_headers} == "external":
        headers = {
            header for header in headers if _file_is_in_main_workspace_and_not_external(header)}
...
def _get_files(compile_action):
    ...
    source_file_candidates = [arg for arg in compile_action.arguments if not arg.startswith(
    ...
    if len(source_file_candidates) > 1:
        ...
        assert source_file.endswith(
            _get_files.source_extensions), f"Source file candidate, {source_file}, seems to be wrong.\nSelected from {compile_action.arguments}.\nPlease file an issue with this information!"
    ...
    if (any(header_file.endswith('.h') for header_file in header_files)
        ...
        if compile_action.arguments[0].endswith('cl.exe'):
            ...
            lang_flag = _get_files.extensions_to_language_args[os.path.splitext(source_file)[
                1]]
...
_get_files.source_extensions = _get_files.c_source_extensions + _get_files.cpp_source_extensions + _get_files.objc_source_extensions + _get_files.objcpp_source_extensions + _get_files.cuda_source_extensions + \
    _get_files.opencl_source_extensions + _get_files.openclxx_source_extensions + \
...
_get_files.extensions_to_language_args = {
    ext: flag for exts, flag in _get_files.extensions_to_language_args.items() for ext in exts}  # Flatten map for easier use
...
def _apple_platform_patch(compile_args: typing.List[str]):
    ...
    if any('__BAZEL_XCODE_' in arg for arg in compile_args):
        ...
        compile_args = [arg for arg in compile_args if not arg.startswith(
            'DEBUG_PREFIX_MAP_PWD') or arg == 'OSO_PREFIX_MAP_PWD']
        ...
        compile_args = [arg.replace(
            '__BAZEL_XCODE_DEVELOPER_DIR__', _get_apple_DEVELOPER_DIR()) for arg in compile_args]
        apple_platform = _get_apple_platform(compile_args)
        assert apple_platform, f"Apple platform not detected in CMD: {compile_args}"
        compile_args = [arg.replace('__BAZEL_XCODE_SDKROOT__', _get_apple_SDKROOT(
            apple_platform)) for arg in compile_args]
...
def _emscripten_platform_patch(compile_action):
    ...
    if 'EM_CONFIG_PATH' not in environment:
        environment['EM_CONFIG_PATH'] = str(get_workspace_root(
            emcc_driver) / 'emscripten_toolchain' / 'emscripten_config')
...
def _all_platform_patch(compile_args: typing.List[str]):
    ...
    compile_args = (
        arg for arg in compile_args if not arg.startswith('-fdebug-prefix-map'))
    ...
    compile_args = (arg for arg in compile_args if not arg ==
    ...
    if os.path.islink(compile_args[0]):
        # MIN_PY=3.9 Switch to pathlib path.readlink()
    ...
    for arg in compile_args:
        ...
        for flag_with_arg in _nvcc_flags_to_skip_with_arg:
            if arg == flag_with_arg:
                ...
                skip_next = True
                break
            # NVCC allows all one-character flags to have no = or space separator before their arguments
        ...
        if ',' in arg:  # Unpack NVCC's (fairly unique) comma-separated list format
            if arg.startswith('-'):
                option, did_partition, remainder = arg.partition('=')
                # -D has a special -D=name=value case.
                if did_partition and not arg.startswith('-D'):
                    arg = remainder
...
_nvcc_flags_to_skip_no_arg = {
    ...
    # Not handled quite right--but very hard to handle fully. If patching, consider adding a special case prepending '-Wl,', making sure to not trigger the list expander.
...
def _get_cpp_command_for_files(compile_action):
    ...
    compile_action.environmentVariables = {pair.key: pair.value for pair in getattr(
        compile_action, 'environmentVariables', [])}
    # Bazel only adds if --incompatible_strict_action_env is passed--and otherwise inherits.
...
def _convert_compile_commands(aquery_output):
    ...
    if {exclude_headers} == "external" and not {exclude_external_sources}:
        targets_by_id = {
            target.id: target.label for target in aquery_output.targets}
        for action in aquery_output.actions:
            ...
            assert not target.startswith(
                '//external'), f"Expecting external targets will start with @. Found //external for action {action}, target {target}"
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        outputs = threadpool.map(
            _get_cpp_command_for_files, aquery_output.actions)
...
def _get_commands(target: str, flags: str):
    ...
    aquery_args = [
        ...
        f"mnemonic('FileWrite|AvCcCompile|CudaDeviceLink|LinkExec',{target_statment})",
    ...
    aquery_process = subprocess.run(
        ...
        stderr=subprocess.PIPE,
        encoding=locale.getpreferredencoding(),
        # We explicitly ignore errors from `bazel aquery` and carry on.
        check=False,
    )
    ...
    aquery_process.stderr = '\n'.join(line for line in aquery_process.stderr.splitlines(
    ) if not missing_targets_warning.match(line))
    if aquery_process.stderr:
        print(aquery_process.stderr, file=sys.stderr)
...
def _ensure_external_workspaces_link_exists():
    ...
    if os.path.lexists(source):
        ...
        try:
            # MIN_PY=3.9 source.readlink()
            current_dest = pathlib.Path(os.readlink(source))
        except OSError:
    ...
    if not os.path.lexists(source):
        if is_windows:
            ...
            # shell required for mklink builtin
            subprocess.run(
                f'mklink /J "{source}" "{dest}"', check=True, shell=True)
        else:
...
def _ensure_gitignore_entries_exist():
    ...
    git_dir_process = subprocess.run('git rev-parse --git-common-dir',  # common-dir because despite current gitignore docs, there's just one info/exclude in the common git dir, not one in each of the worktree's git dirs.
                                     # Ensure this will still fail with a nonzero error code even if `git` isn't installed, unifying error cases.
                                     shell=True,
